...
from common.tenant_scope import scope_queryset, assign_tenant, enforce_tenant
...
def clinic_note_create(request):
    ...
    if request.method == "POST":
        ...
        if note_type:
            ...
            if patient_id and note_content:
                ClinicalRecord.objects.create(
                    tenant=request.user.tenant if not request.user.platform_admin else patients.filter(pk=patient_id).first().tenant,
        ...
        template = CLINIC_NOTE_TEMPLATES.get(specialization, CLINIC_NOTE_TEMPLATES.get('general_practice', []))
