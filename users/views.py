...
from billing.free_trial import (
    ...
    is_free_patient_limit_reached,
    is_free_user_limit_reached,
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            log_audit(
                ...
                user=user,
                tenant=tenant,
                details=f"User {user.username} registered for tenant {tenant}.",
            )

            # Handle based on registration type
            if registration_type == "create":
                # New company created - user is admin and automatically activated
                messages.success(
                    request,
                    f"Welcome! Your company '{tenant.name}' has been created successfully. "
                    f"You have {free_trial_days_left(tenant)} days of free trial with limits: 2 users, 5 patients. "
                    f"You can now log in as an administrator.",
                )
                return redirect("login")
            else:
                # Joining existing company - notify admins
                if is_tenant_in_free_trial(tenant):
                    if is_free_user_limit_reached(tenant):
                        # Delete the user since we can't add them
                        user.delete()
                        upgrade_message = mark_safe(
                            'Free trial limit: max 2 users. <a href="/billing/" class="alert-link">Company needs to upgrade to add more users</a>.'
                        )
                    ...
                    # User successfully registered, notify admins
                    notify_admins_of_new_user(user)
                    messages.success(
                        request,
                        f"Registration successful! An email has been sent to {tenant.name} administrators for approval. "
                        f"You will receive an email once your account is activated.",
                    )
                    return redirect("login")
                else:
                    # Free trial expired
                    ...
                    messages.error(
                        request,
                        f"{tenant.name}'s free trial has expired. Please contact the company administrator to upgrade.",
                    )
...
class TenantPasswordResetView(auth_views.PasswordResetView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request, "If the email exists, a password reset link has been sent."
        )
...
class TenantPasswordResetConfirmView(auth_views.PasswordResetConfirmView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request, "Your password has been set. You may now log in."
        )
        return super().form_valid(form)
...
class TenantLoginView(LoginView):
    ...
    def form_valid(self, form):
        ...
        user = authenticate(
            self.request, username=username, password=password, tenant=tenant
        )
        if user is not None:
            ...
            try:
                ...
                if not inactive_user.is_active and inactive_user.check_password(
                    password
                ):
                    contact_admin_link = mark_safe(
                        "Your account is pending admin approval. "
                        '<a href="mailto:admin@cliniccloud.com?subject=Account%20Approval%20Request&body=I%20would%20like%20to%20request%20approval%20for%20my%20ClinicCloud%20account.%20Username:%20'
                        + username
                        + '" '
                    ...
                    logger.info(
                        f"Inactive user '{username}' attempted login for tenant '{tenant}'."
                    )
            ...
            logger.error(
                f"Login error for user '{username}' and tenant '{tenant}': Invalid credentials or tenant."
            )
