    ...
    def handle(self, *args, **options):
        ...
        if options.get("tenant_id"):
            try:
                ...
                self.stdout.write(
                    f"✓ Using existing tenant: {tenant.name} (ID: {tenant.id})"
                )
            except Tenant.DoesNotExist:
                raise CommandError(
                    f"Tenant with ID {options['tenant_id']} does not exist"
                )
            ...
            if existing_tenants:
                ...
                tenant_id = input(
                    "\nEnter tenant ID (or 'new' to create new): "
                ).strip()
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"✓ Created default tenant: {tenant.name}")
                )
