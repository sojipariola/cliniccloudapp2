...
class CustomUserAdmin(admin.ModelAdmin):
    ...
    def activate_link(self, obj):
        ...
        return format_html(
            '<span style="color:#10b981;font-weight:600;">{}</span>', "✓ Active"
        )
    ...
    def grant_platform_admin(self, request, queryset):
        ...
        for user in queryset:
            if not user.platform_admin:
                ...
                updated += 1
        self.message_user(
            request, f"{updated} user(s) promoted to ClinicCloud platform admin."
        )
    ...
    def revoke_platform_admin(self, request, queryset):
        ...
        for user in queryset:
            if user.platform_admin:
                ...
                updated += 1
        self.message_user(
            request, f"{updated} user(s) demoted from ClinicCloud platform admin."
        )
